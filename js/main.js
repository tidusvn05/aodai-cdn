...
var language = {'JP':{'quan_1':'1区','quan_3':'3区','quan_binhthanh':'ビンタン区','quan_tanbinh':'タンビン区','quan_phunhuan':'フーニャン区','quan_7':'7区','quan_7_H':'7区フーミンフン','quan_khac':'その他の区','le_thanh_ton':'1区レタントン日本人街近隣','quan_2':'2区','quan_4':'4区','quan_tanphu':'タンフー区', 'quan_govap':'ゴーバップ区','quan_binhtan':'BINH TAN区','quan_5':'5区','quan_6':'6区','quan_8':'8区','quan_9':'9区','quan_10':'10区', 'quan_11':'11区','quan_12':'12区'}
    ,'VN':{'quan_1':'1区','quan_3':'3区','quan_binhthanh':'ビンタン区','quan_tanbinh':'タンビン区','quan_phunhuan':'フーニャン区','quan_7':'7区','quan_7_H':'7区フーミンフン','quan_khac':'その他の区','le_thanh_ton':'1区レタントン日本人街近隣','quan_2':'2区','quan_4':'4区','quan_tanphu':'タンフー区', 'quan_govap':'ゴーバップ区','quan_binhtan':'BINH TAN区','quan_5':'5区','quan_6':'6区','quan_8':'8区','quan_9':'9区','quan_10':'10区', 'quan_11':'11区','quan_12':'12区'}
};
...
AODAI.prototype.menuHeaderClick = function() {
    ...
    $(".sub-menu-list-header a").click(function(e){
...
AODAI.prototype.scrollbar = function(callback){  
    $(".style-3").mCustomScrollbar({
        theme:"dark-3" // horizontal scrollbar
    });  
    setTimeout(function(){ 
        $(".lSPager.lSGallery").mCustomScrollbar({
        theme:"dark-3" // horizontal scrollbar
        }); 
    }, 10);
}
...
AODAI.prototype.changerangePrice = function() {
    ...
    if(rangebar.length === 1) {     
        buttonStart = $(".range-bar").offset().left + 26; 
        if(inputMax === null){
            inputMax=max_price_tooltip;
        }
        var arrValues = [inputMin,inputMax];
        choosePrice(position_min_price, position_max_price, inputMin, inputMax);  
    }

    $('.price-sub-menu li').on('click',function(){
        total_price = $(this).data('id').toString();      
...
AODAI.prototype.chooseLanguage = function() {
    $('.block-menu-language li a').click(function() {
        ...
        $(this).parent().addClass('active');
        if($(this).find('font font').text() == "VN"){
            langSelected = language.VN;
        }
        else {
            langSelected = language.JP;
        }
    });
}
...
AODAI.prototype.confirmInfoClick = function(){
    $('#myModal').on('show.bs.modal', function (e) {
        ...
        // Get name all checkbox first
        var name_checked_first = [];
        $(".checkbox-block-first input:checked").each(function() {
            name_checked_first.push($(this).next().text());

        });
        $('.list').append('<li>'+$('.checkbox-label-block-first').text()+': '+name_checked_first.join(',\t')+'</li>');

        // Get name all checkbox second
        var name_checked_second = [];
        $(".checkbox-block-second input:checked").each(function() {
            name_checked_second.push($(this).next().text());
...
AODAI.prototype.validateConfirmInfoClick = function(){
    $('#myModal').modal();
}

/**
 * Validate form estate owners
 */
AODAI.prototype.validateEstateOwners = function() {

    // Default remove all error message of input
    ...
    $('.info-input .inputRequire').each(function(i, element) {
        ...
        if (!value.trim() && i!= 7 ) {
            // Add message error check input after input
            ...
            if (i && ( i == 2 || i == 6 || i == 8)) {
                if (isNaN(value)) {
                    // Add message error check number after input
    ...
    result = false;
    if(!isErrorInput && !isErrorNumber && isEmail) {
         result = true;
    } else {
        // Scroll to username
        $('.info-input .error').each(function (i, element) {
            var a = $(element).parent().prev().attr('id');
            if ($(element).parent().prev().attr('id') === 'scroll-position-username' ) {
                scrollToElement('scroll-position-username');
                return false;
            }
            if ($(element).parent().prev().attr('id') === 'scroll-position-email') {
                scrollToElement('scroll-position-email');
                return false;
            }
            if($(element).parent().prev().attr('id') === 'scroll-position-phone') {
                scrollToElement('scroll-position-phone');
                return false;
            }
            if($(element).parent().prev().attr('id') === 'scroll-position-address') {
                scrollToElement('scroll-position-address');
                return false;
            }
            if($(element).parent().prev().attr('id') === 'scroll-position-name-en') {
                scrollToElement('scroll-position-name-en');
                return false;
            }
            if($(element).parent().prev().attr('id') === 'scroll-position-name-jp') {
                scrollToElement('scroll-position-name-jp');
                return false;
            }
            if($(element).parent().prev().attr('id') === 'scroll-position-rent') {
                scrollToElement('scroll-position-rent');
                return false;
            }
            if($(element).parent().prev().attr('id') === 'scroll-position-bedroom') {
                scrollToElement('scroll-position-bedroom');
                return false;
            }
            if($(element).parent().prev().attr('id') === 'scroll-position-deposit') {
                scrollToElement('scroll-position-deposit');
                return false;
            }
            if($(element).parent().prev().attr('id') === 'scroll-position-bathroom') {
                scrollToElement('scroll-position-bathroom');
                return false;
            }
            if($(element).parent().prev().attr('id') === 'scroll-position-area') {
                scrollToElement('scroll-position-area');
                return false;
            }
            if($(element).parent().prev().attr('id') === 'scroll-position-reach') {
                scrollToElement('scroll-position-reach');
                return false;
            }
            if($(element).parent().prev().attr('id') === 'scroll-position-type') {
                scrollToElement('scroll-position-type');
                return false;
            }
            if($(element).parent().prev().attr('id') === 'scroll-position-size') {
                scrollToElement('scroll-position-size');
                return false;
            }
        })
    }
...
AODAI.prototype.showtextUpload = function() {
        ...
        var scrollTrigger = 100, // px
            backToTop = function() {
                ...
                if (scrollTop > scrollTrigger) {
                    $('.btn-to-top, .link-to-top').addClass('showTop');
                } else {
                    $('.btn-to-top, .link-to-top').removeClass('showTop');
                }
            };
        backToTop();
...
$(window).click(function() {
    ...
    if($('.btn-menu').hasClass('btn-close-menu')) {
        $('.btn-menu').trigger('click');
    }
});

AODAI.prototype.sliderPrimary = function() {
    $('.slider-top-menu').bxSlider({
...
AODAI.prototype.staffList = function() {
...
AODAI.prototype.slideDetailProduct = function() {
...
AODAI.prototype.sliderSearchResult = function() {
    $('.img-product-item').owlCarousel({
        items: 1,
        nav: true,
        loop: true
    });
...
AODAI.prototype.sliderSearchResult2 = function() {
    ...
    hover = function(polygon) {
        ...
        polygon.mouseout(function() {
            if (this.hasClass('active')) {
                ...
                this.children()[0].attr('fill', '#E8E8E8');
                if (this.children()[1])
                    this.children()[1].fill('#84292D');
            }
        })
...
AODAI.prototype.InformactionArea = function () {
    ...
    targer.click(function () {
        ...
        contentImagesArea.each(function (index) {
            if($(this).hasClass('active')){
                $(this).removeClass('active');
            }
        });
...
AODAI.prototype.activeDefaultProduct = function(element) {
    $(element).addClass('active');
}
...
AODAI.prototype.wowAnimate = function() {
    wowActive = function(wow, classUse) {
        wow = new WOW({
            boxClass: classUse, // animated element css class (default is wow)
    ...
    if (classArivalBlock.length === 1)
        wowActive(arivalBlock, 'arrival-block');
    var benefitBlock = null,
        classBenefitBlock = $('.benefit-block');
    if (classBenefitBlock.length === 1)
        wowActive(benefitBlock, 'benefit-block');
    var featureBlock = null,
        classFeatureBlock = $('.feature-block');
    if (classFeatureBlock.length === 1)
        wowActive(featureBlock, 'feature-block');
    var supportBlock = null,
        classSupportBlock = $('.support-block');
    if (classSupportBlock.length === 1)
        wowActive(supportBlock, 'support-block');
    var subIntroduce = null,
        classSubIntroduce = $('.sub-introduce-staff');
    if (classSubIntroduce.length === 1)
        wowActive(subIntroduce, 'sub-introduce-staff');
    var operationCompany = null,
        classOperationCompany = $('.operation-company');
    if (classOperationCompany.length === 1)
        wowActive(operationCompany, 'operation-company');
    var webInfo = null,
        classWebInfo = $('.website-information');
    if (classWebInfo.length === 1)
        wowActive(webInfo, 'website-information');
    var productFeatures = null,
        classProductFeatures = $('.product-features');
    if (classProductFeatures)
        wowActive(productFeatures, 'product-features');
    var productBlock = null,
        classProductBlock = $('.product-block');
    if (classProductBlock)
        wowActive(productBlock, 'product-block');
    var economyItem = null,
        classEconomyItem = $('.economical-items');
    if (classEconomyItem)
        wowActive(economyItem, 'economical-items');
    var ecoStep = null,
        classEcoStep = $('.eco-steps-block');
    if (classEcoStep)
        wowActive(ecoStep, 'eco-steps-block');
    var introStaff = null,
        classIntroStaff = $('.intro-staff-eco');
    if (classIntroStaff)
        wowActive(introStaff, 'intro-staff-eco');
    var mapContent = null,
        classMapContent = $('.map-content');
    if (classMapContent)
        wowActive(mapContent, 'map-content');
    var apartmentFeature =  null,
        classAparment = $('.three-feature-appartment-home');
        if(classAparment)
            wowActive(apartmentFeature,'three-feature-appartment-home');
    // var introduceDist =  null,
    //     classIntroDist = $('.introduce-district-wrapper');
    //     if(classIntroDist)
    //         wowActive(introduceDist,'introduce-district-wrapper');
    var apartmentRedirect =  null,
        classRedirect = $('.three-redirect-appartment-banner');
        if(classRedirect)
            wowActive(apartmentRedirect,'three-redirect-appartment-banner');
}
...
function resize_change(is_first){
    ...
    if($width < 992){
        $('.wrap-search-box').appendTo('header .menu-header .top-menu');
        $('.wrap-search-box').css('display','none');
        $('.search-box').removeClass('active');
    }
    else{
        $('.wrap-search-box').insertBefore('voucher-block').css('display','');
        $('.wrap-search-box').find('.search-box').addClass('active');
    }
}
...
function showSearchForm(){
    $('.search_home_custom').on('click', function(event){
        event.preventDefault();
        $(this).find('.btn-open').toggleClass('fa-angle-down fa-angle-up');
        if($(this).hasClass("search_home_custom")){
            $(this).find('span').toggleClass('fa-angle-down fa-times-circle-o');
            $(this).find('span').toggleClass('ico-down ico-close');
        }
        $('body header .menu-header .top-menu').toggleClass('toggle-search');
        if($(window).width < 992) $('html body').scrollTop(0);
        else $('body').toggleClass('open');
        $('.wrap-search-box').fadeToggle('fast', 'linear');
    });
}
showSearchForm();
